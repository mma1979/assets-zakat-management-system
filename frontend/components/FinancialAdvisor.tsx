...
export const FinancialAdvisor: React.FC<FinancialAdvisorProps> = ({ data }) => {
  ...
  const holdings = useMemo(() => {
    ...
    types.forEach(type => {
      ...
      let currentPrice = 0;
      if (type === 'GOLD') currentPrice = data.rates.gold_egp;
      if (type === 'GOLD_21') currentPrice = data.rates.gold21_egp;
      if (type === 'SILVER') currentPrice = data.rates.silver_egp;
      if (type === 'USD') currentPrice = data.rates.usd_egp;
